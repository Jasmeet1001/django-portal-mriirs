...
def import_excel(file):
    ...
    col_include = [ 'faculty', 'authors', 'outside author', 'domain', 'title of paper', 'dept.', 'name of journal', 'name of conference', 'title of book', 'title of chapter', 'student', 'scholar', 'publication month', 'publication year', 'doi', 'scopus id']
    try:
        with pd.ExcelFile(f'./media/{filename}') as file_to_load:
            for sheet in file_to_load.sheet_names:
                ...
                for rows in data_new.itertuples():
                    ...
                    tbl_val = ResearchPaper.objects.create(faculty=rows[1], authors=f'{authors}', domain=rows[4], title_of_paper=rows[5], dept=rows[6], name_of_journal=rows[7], name_of_conference=rows[8], title_of_book=rows[9], title_of_chapter=rows[10], student=rows[11], scholar=rows[12], month=rows[13], year=rows[14])
