                ...
                with transaction.atomic():                    
                    for rows in data.itertuples():
                        if (ResearchPaper.objects.filter(title_of_paper__iexact = str(rows[5])).exists()):
                            ...
                            if len(rows[1]) > 5:
                                    raise CustomExceptions(f"{message} Possible error in row {rows[0] + 1}")
                            elif not str(rows[2]).strip():
                                raise CustomExceptions(f"{message} Authors field cannot be empty in row {rows[0] + 1}")
                            elif not str(rows[5]).strip():
                                raise CustomExceptions(f"{message} Title of Papers field cannot be empty in row {rows[0] + 1}")
                            elif len(rows[11]) > 3:
                                raise CustomExceptions(f"{message} Possible error in row {rows[0] + 1}")
                            elif len(rows[12]) > 3:
                                raise CustomExceptions(f"{message} Possible error in row {rows[0] + 1}")
                            elif str(rows[13]).isdigit():
                                raise CustomExceptions(f"{message} Month should not be in number in row {rows[0] + 1}")
                            elif str(rows[14]).strip() and not('-' in str(rows[14]) and len(str(rows[14]).split('-')[0]) == 4 and len(str(rows[14]).split('-')[1]) == 4):
                                raise CustomExceptions(f"{message} Year column should be in the format of YYYY-YYYY in row {rows[0] + 1}")
                            elif str(rows[15]).strip():
                                try:
                                    validate = URLValidator(schemes=['doi', 'http', 'https', 'fpt', 'ftps'])
                                    validate(str(rows[15]))
                                except ValidationError:
                                    raise CustomExceptions(f"{message} Provided doi is not a valid url in row {rows[0] + 1}")
                            else:
